...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  return (
    <div className="group bg-white rounded-none overflow-hidden transition-all duration-1000 flex flex-col h-full relative border border-transparent hover:border-brand-gold/10">
      ...
      <Link to={`/product/${product.id}`} className={`block relative overflow-hidden aspect-[4/5] p-10 border border-brand-green/10 ${product.category === 'Whole Spices' ? 'bg-white' : 'bg-[#FBFBFA]'}`}>
        <img
          src={product.image}
          alt={product.name}
